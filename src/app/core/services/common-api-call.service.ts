...
export class CommonApiCallService {
  ...
  generateOtp(mobileNo:any){
    return new Observable((obj) => {
      ...
      this.apiService.getHttp().subscribe({
        next: (res: any) => { if (res.statusCode == 200) { this.otp = res.responseData; obj.next(this.otp); } else { obj.error(res); } },
        error: (e: any) => { obj.error(e) }
      })
    })
  }
  ...
  verifyOtp(mobileNo:any,otp:any){
    return new Observable((obj) => {
      ...
      this.apiService.getHttp().subscribe({
        next: (res: any) => { if (res.statusCode == 200) { this.verify = res.responseData; obj.next(this.verify); } else { obj.error(res); } },
        error: (e: any) => { obj.error(e) }
      })
    })
  }
